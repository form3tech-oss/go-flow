...
scan-code: require-snyk-token-env require-travis-env aws-auth-eu-west-1
	...
	@exec docker run --rm -v $(shell pwd):/code -e GOFLAGS='-mod=vendor' -e TRAVIS -e REPO=form3tech-oss/go-flow -e SNYK_TOKEN=${SNYK_TOKEN} "288840537196.dkr.ecr.eu-west-1.amazonaws.com/tech.form3/secscan-go-golang:latest"
